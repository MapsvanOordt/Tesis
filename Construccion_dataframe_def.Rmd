---
title: "Construcción Base de datos"
...
cc4 = cc3[cc3$DISTRITO != "EN INVESTIGACIÓN", ]
cc4$UBIGEO=as.factor(cc4$UBIGEO)#1852 únicos
str(cc4$UBIGEO)
```
...
group_by(mes,drop = FALSE)
merge4$mes=as.factor(merge4$mes)
merge5$UBIGEO=as.factor(merge5$UBIGEO)
merge5$year=as.factor(merge5$year)

merge5 %>%  count(mes,UBIGEO) %>% count(mes)
merge5 %>% arrange(year,UBIGEO,mes) %>% view()
write_csv(merge4, path = "merge4.csv")
...
UNION DATA_CENSO_CLUS A MERGE 4
```{r}
DATA <- full_join(merge4,data_censo,by = "ubigeo") %>% view()
...
+ df %>% count(EJECUTORA_NOMBRE)
Error in df(.) : argument "df1" is missing, with no default
> df %>% count(EJECUTORA_NOMBRE) %>% view()
> df %>% count(FUNCION_NOMBRE) %>% view()
> df %>% count(FUENTE_FINANC_NOMBRE) %>% View()
> df %>% count(RUBRO_NOMBRE,FUENTE_FINANC_NOMBRE) %>% view()

> df %>% count(EJECUTORA_NOMBRE) %>% View()
> df %>% count(META_NOMBRE,FUNCION_NOMBRE) %>% View()
> df %>% count(DIVISION_FUNCIONAL_NOMBRE,FUNCION_NOMBRE) %>% View()
> df %>% View()
> > df %>% count(ACTIVIDAD_ACCION_OBRA_NOMBRE) %>% View()
Error: unexpected '>' in ">"
> df %>% count(ACTIVIDAD_ACCION_OBRA_NOMBRE) %>% View()
> 
